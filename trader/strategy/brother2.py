...
class TradeStrategy(BaseModule):
    def __init__(self, name: str):
        super().__init__()
        self.__market_response_format = config.get(
            'MSG_CHANNEL', 'market_response_format')
        self.__trade_response_format = config.get(
            'MSG_CHANNEL', 'trade_response_format')
        self.__request_format = config.get('MSG_CHANNEL', 'request_format')
        self.__ignore_inst_list = config.get(
            'TRADE', 'ignore_inst', fallback="WH,bb,JR,RI,RS,LR,PM,im").split(',')
        self.__strategy = Strategy.objects.get(name=name)
        self.__inst_ids = self.__strategy.instruments.all(
        ...
        self.__re_extract_code = re.compile(
            r'([a-zA-Z]*)(\d+)')  # 提合约字母部分 IF1509 -> IF
        self.__re_extract_name = re.compile('(.*?)([0-9]+)(.*?)$')  # 提取合约文字部分
        self.__trading_day = timezone.make_aware(datetime.datetime.strptime(
            self.raw_redis.get("TradingDay")+'08', '%Y%m%d%H'))
        self.__last_trading_day = timezone.make_aware(datetime.datetime.strptime(
            self.raw_redis.get("LastTradingDay")+'08', '%Y%m%d%H'))
        ...
        if today.isoweekday() < 6 and (820 <= now <= 1550 or 2010 <= now <= 2359):  # 非交易时间查不到数据
            ...
            for order in order_list:
                ...
                if int(order['OrderStatus']) in range(1, 5) and order['OrderSubmitStatus'] == ApiStruct.OSS_Accepted:
                    ...
                    logger.info(
                        f"撤销未成交订单: 合约{order['InstrumentID']} {direct_str}单 {order['VolumeTotal']}手 价格{order['LimitPrice']}")
    ...
    async def refresh_account(self):
        try:
            ...
            if fake < 1:
                fake = 0
            # 静态权益=上日结算+入金金额-出金金额
            self.__pre_balance = Decimal(
                account['PreBalance']) + self.__deposit - self.__withdraw
            ...
            self.__broker.save(
                update_fields=['cash', 'current', 'pre_balance', 'margin'])
    ...
    async def refresh_position(self):
        try:
            ...
            Trade.objects.filter(~Q(code__in=self.__cur_pos.keys(
            )), close_time__isnull=True).delete()  # 删除不存在的头寸
            for _, pos in self.__cur_pos.items():
                p_code = self.__re_extract_code.match(
                    pos['InstrumentID']).group(1)
                ...
                bar = DailyBar.objects.filter(
                    code=pos['InstrumentID']).order_by('-time').first()
                profit = (
                    bar.close - Decimal(pos['OpenPrice'])) * pos['Volume'] * inst.volume_multiple
                if pos['Direction'] == DirectionType.values[DirectionType.SHORT]:
                    profit *= -1
                if trade:
                    trade.shares = (
                        trade.closed_shares if trade.closed_shares else 0) + pos['Volume']
                    trade.filled_shares = trade.shares
                    trade.profit = profit
                    trade.save(update_fields=[
                               'shares', 'filled_shares', 'profit'])
                else:
                    Trade.objects.create(
                        broker=self.__broker, strategy=self.__strategy, instrument=inst, code=pos[
                            'InstrumentID'], profit=profit, filled_shares=pos['Volume'],
        ...
        try:
            ...
            for inst in inst_list:
                ...
                if inst['IsTrading'] == 1 and chr(inst['ProductClass']) == ApiStruct.PC_Futures and int(str_to_number(inst['StrikePrice'])) == 0:
                    ...
                    inst_dict[inst['ProductID']][inst['InstrumentID']
                                                 ]['name'] = inst['InstrumentName']
                    inst_dict[inst['ProductID']][inst['InstrumentID']
                                                 ]['exchange'] = inst['ExchangeID']
                    inst_dict[inst['ProductID']][inst['InstrumentID']
                                                 ]['multiple'] = inst['VolumeMultiple']
                    inst_dict[inst['ProductID']][inst['InstrumentID']
            ...
            for code in inst_dict.keys():
                ...
                inst, created = Instrument.objects.update_or_create(
                    product_code=code, exchange=inst_data['exchange'])
                print(
                    f"inst:{inst} created:{created} main_code:{inst.main_code}")
            ...
            logger.warning(
                f'refresh_instrument 发生错误: {repr(e)}', exc_info=True)
    ...
    def getShares(self, instrument: str):
        ...
        for pos in self.__shares[instrument]:
            ...
            shares += pos['Volume'] * \
                (-1 if pos['Direction'] == DirectionType.SHORT else 1)
    ...
    def async_query(self, query_type: str, **kwargs):
        ...
        self.raw_redis.publish(self.__request_format.format(
            'ReqQry' + query_type), json.dumps(kwargs))
    ...
    async def query(self, query_type: str, **kwargs):
        ...
        try:
            sub_client = self.redis_client.pubsub(
                ignore_subscribe_messages=True)
            request_id = get_next_id()
            kwargs['RequestID'] = request_id
            channel_rsp_qry = self.__trade_response_format.format(
                'OnRspQry' + query_type, request_id)
            channel_rsp_err = self.__trade_response_format.format(
    ...
    async def SubscribeMarketData(self, inst_ids: list):
        ...
        try:
            sub_client = self.redis_client.pubsub(
                ignore_subscribe_messages=True)
            channel_rsp_dat = self.__market_response_format.format(
                'OnRspSubMarketData', 0)
            channel_rsp_err = self.__market_response_format.format(
                'OnRspError', 0)
            await sub_client.psubscribe(channel_rsp_dat, channel_rsp_err)
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format(
                'SubscribeMarketData'), json.dumps(inst_ids))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
            return task.result()
            ...
            logger.warning(
                f'SubscribeMarketData 发生错误: {repr(e)}', exc_info=True)
    ...
    async def UnSubscribeMarketData(self, inst_ids: list):
        ...
        try:
            sub_client = self.redis_client.pubsub(
                ignore_subscribe_messages=True)
            channel_rsp_dat = self.__market_response_format.format(
                'OnRspUnSubMarketData', 0)
            channel_rsp_err = self.__market_response_format.format(
                'OnRspError', 0)
            await sub_client.psubscribe(channel_rsp_dat, channel_rsp_err)
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format(
                'UnSubscribeMarketData'), json.dumps(inst_ids))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
            return task.result()
            ...
            logger.warning(
                f'UnSubscribeMarketData 发生错误: {repr(e)}', exc_info=True)
            if sub_client and sub_client.in_pubsub and channel_rsp_dat:
                await sub_client.unsubscribe()
                await sub_client.close()
            return None
        ...
        try:
            ...
            match sig.type:
                case SignalType.BUY | SignalType.SELL_SHORT:
                    ...
                    logger.info(
                        f'{sig.instrument} {sig.type}{sig.volume}手 价格: {sig.price}')
                ...
                case SignalType.ROLL_CLOSE:
                    ...
                    pos = Trade.objects.filter(broker=self.__broker, strategy=self.__strategy,
                                               code=sig.code, shares=sig.volume, close_time__isnull=True).first()
                    param_dict['Direction'] = ApiStruct.D_Sell if pos.direction == DirectionType.values[DirectionType.LONG] else ApiStruct.D_Buy
                    if pos.open_time.astimezone().date() == timezone.localtime().date() and pos.instrument.exchange == ExchangeType.SHFE:
                        # 上期所区分平今和平昨
                        param_dict['CombOffsetFlag'] = ApiStruct.OF_CloseToday
                    logger.info(
                        f'{sig.code}->{sig.instrument.main_code} {pos.direction}头换月平旧{sig.volume}手 价格: {sig.price}')
                case SignalType.ROLL_OPEN:
                    param_dict['CombOffsetFlag'] = ApiStruct.OF_Open
                    pos = Trade.objects.filter(
                        Q(close_time__isnull=True) | Q(
                            close_time__date__gte=timezone.localtime().now().date()),
                    ...
                    logger.info(
                        f'{pos.code}->{sig.code} {pos.direction}头换月开新{sig.volume}手 价格: {sig.price}')
            self.raw_redis.publish(self.__request_format.format(
                'ReqOrderInsert'), json.dumps(param_dict))
        except Exception as e:
    ...
    async def cancel_order(self, order: dict):
        ...
        try:
            ...
            channel_rtn_odr = self.__trade_response_format.format(
                'OnRtnOrder', order['OrderRef'])
            channel_rsp_odr_act = self.__trade_response_format.format(
                'OnRspOrderAction', 0)
            channel_rsp_err = self.__trade_response_format.format(
                'OnRspError', request_id)
            ...
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format(
                'ReqOrderAction'), json.dumps(order))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
    ...
    async def OnRtnDepthMarketData(self, channel, tick: dict):
        try:
            ...
            tick['UpdateTime'] = datetime.datetime.strptime(
                tick['UpdateTime'], "%Y%m%d %H:%M:%S:%f")
            logger.debug('inst=%s, tick: %s', inst, tick)
        except Exception as ee:
            logger.warning('OnRtnDepthMarketData 发生错误: %s',
                           repr(ee), exc_info=True)
    ...
    def get_trade_string(trade: dict) -> str:
        if trade['OffsetFlag'] == OffsetFlag.Open:
            ...
            open_direct = DirectionType.values[DirectionType.LONG] if trade[
                'Direction'] == DirectionType.SHORT else DirectionType.values[DirectionType.SHORT]
        ...
        try:
            ...
            if not manual_trade:
                signal = Signal.objects.get(
                    id=int(order_ref[ORDER_REF_SIGNAL_ID_START:]))
            logger.info(f"成交回报: {self.get_trade_string(trade)}")
            inst = Instrument.objects.get(
                product_code=self.__re_extract_code.match(trade['InstrumentID']).group(1))
            order = Order.objects.filter(
                order_ref=order_ref, code=trade['InstrumentID']).order_by('-send_time').first()
            trade_cost = trade['Volume'] * Decimal(
                trade['Price']) * inst.fee_money * inst.volume_multiple + trade['Volume'] * inst.fee_volume
            trade_margin = trade['Volume'] * \
                Decimal(trade['Price']) * inst.margin_rate
            now = timezone.localtime()
            trade_time = timezone.make_aware(datetime.datetime.strptime(
            ...
            if trade['OffsetFlag'] == OffsetFlag.Open:  # 开仓
                last_trade = Trade.objects.filter(
                    broker=self.__broker, strategy=self.__strategy, instrument=inst, code=trade[
                        'InstrumentID'], open_time__lte=trade_time,
                ...
                if last_trade is None:
                    ...
                    last_trade = Trade.objects.create(
                        broker=self.__broker, strategy=self.__strategy, instrument=inst, code=trade[
                            'InstrumentID'], open_order=order if order else None,
                        direction=DirectionType.values[trade['Direction']
                                                       ], open_time=trade_time, shares=order.volume if order else trade['Volume'],
                ...
                if last_trade:
                    if last_trade.closed_shares and last_trade.avg_exit_price:
                        last_trade.avg_exit_price = (last_trade.avg_exit_price * last_trade.closed_shares + trade['Volume'] * Decimal(trade['Price'])) / \
                                                    (last_trade.closed_shares +
                                                     trade['Volume'])
                        last_trade.closed_shares += trade['Volume']
                    else:
                        last_trade.avg_exit_price = trade['Volume'] * \
    ...
    def save_order(order: dict):
        try:
            if int(order['OrderRef']) < 10000:  # 非本程序生成订单
                return None, None
            signal = Signal.objects.get(
                id=int(order['OrderRef'][ORDER_REF_SIGNAL_ID_START:]))
            ...
            if order['OrderStatus'] == OrderStatus.Canceled and order['OrderSubmitStatus'] == OrderSubmitStatus.InsertRejected:
                ...
                if order['CombOffsetFlag'] == CombOffsetFlag.Open:
                    if order['Direction'] == DirectionType.LONG:
                        ...
                        price = price_round(
                            last_bar.settlement + delta, inst.price_tick)
        ...
        if trading:
            ...
            if (self.__trading_day - self.__last_trading_day).days > 3:
                ...
                self.io_loop.call_soon(
                    asyncio.create_task, self.processing_signal3())
    ...
    async def collect_quote(self, tasks=None):
        try:
            ...
            if tasks is None:
                tasks = [update_from_shfe, update_from_dce, update_from_czce,
                         update_from_cffex, update_from_gfex, get_contracts_argument]
            ...
            if all(result):
                ...
                failed_tasks = [tasks[i]
                                for i, rst in enumerate(result) if not rst]
                self.io_loop.call_later(
                    10*60, asyncio.create_task, self.collect_quote(failed_tasks))
        except Exception as e:
    ...
    def calc_signal(self, inst: Instrument, day: datetime.datetime) -> (Signal, Decimal):
        try:
            ...
            short_n = self.__strategy.param_set.get(
                code='ShortPeriod').int_value
            ...
            for idx in range(1, df.shape[0]):  # 手动计算SMA
                df.short_trend[idx] = (
                    df.short_trend[idx-1] * (short_n - 1) + df.close[idx]) / short_n
                df.long_trend[idx] = (
                    df.long_trend[idx-1] * (long_n - 1) + df.close[idx]) / long_n
            df["high_line"] = df.close.rolling(window=break_n).max()
            df["low_line"] = df.close.rolling(window=break_n).min()
            idx = -1
            buy_sig = df.short_trend[idx] > df.long_trend[idx] and price_round(
                df.close[idx], inst.price_tick) >= price_round(df.high_line[idx - 1], inst.price_tick)
            sell_sig = df.short_trend[idx] < df.long_trend[idx] and price_round(
                df.close[idx], inst.price_tick) <= price_round(df.low_line[idx - 1], inst.price_tick)
            pos = Trade.objects.filter(close_time__isnull=True, broker=self.__broker,
                ...
                if pos.direction == DirectionType.values[DirectionType.LONG]:
                    ...
                    while hasattr(first_pos, 'open_order') and first_pos.open_order and first_pos.open_order.signal and first_pos.open_order.signal.type == \
                        ...
                        if last_pos is None:
                            break
                        logger.debug(
                            f"发现换月前持仓:{last_pos} 开仓时间: {last_pos.open_time}")
                        first_pos = last_pos
                    pos_idx = df.index.get_loc(
                        first_pos.open_time.astimezone().date().isoformat())
                    # 多头止损
            ...
            # 开新仓
                ...
                profit = Trade.objects.filter(
                    strategy=self.__strategy, instrument__section=inst.section).aggregate(sum=Sum('profit'))['sum']
                profit = profit if profit else 0
                risk_each = Decimal(df.atr[idx]) * \
                    Decimal(inst.volume_multiple)
                volume_ori = (start_cash + profit) * risk / risk_each
                volume = round(volume_ori)
                print(
                    f"{inst}: ({start_cash:,.0f} + {profit:,.0f}) / {risk_each:,.0f} = {volume_ori}")
                if volume > 0:
                    new_bar = DailyBar.objects.filter(
                        exchange=inst.exchange, code=inst.main_code, time=day.date()).first()
                    use_margin = new_bar.settlement * inst.volume_multiple * inst.margin_rate * volume
                    price = self.calc_up_limit(
                        inst, new_bar) if buy_sig else self.calc_down_limit(inst, new_bar)
                    signal = SignalType.BUY if buy_sig else SignalType.SELL_SHORT
                else:
                    logger.info(
                        f"做{'多' if buy_sig else '空'}{inst},单手风险:{risk_each:.0f},超出风控额度，放弃。")
            if signal:
    ...
    def calc_up_limit(self, inst: Instrument, bar: DailyBar):
        ...
        limit_ratio = str_to_number(self.raw_redis.get(
            f"LIMITRATIO:{inst.exchange}:{inst.product_code}:{bar.code}"))
        price_tick = inst.price_tick
        price = price_round(
            settlement * (Decimal(1) + Decimal(limit_ratio)), price_tick)
